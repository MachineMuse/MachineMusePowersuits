package net.machinemuse_old.powersuits.entity;

import net.machinemuse_old.api.ModuleManager;
import net.machinemuse_old.powersuits.powermodule.weapon.BladeLauncherModule;
...
public class EntitySpinningBlade extends EntityThrowable {
    ...
    public EntitySpinningBlade(World par1World, EntityLivingBase shootingEntity) {
        ...
        double yoffset = -.2;
        double zoffset = 0.3f;
        double horzScale = Math.sqrt(direction.x * direction.x + direction.zCoord * direction.zCoord);
        ...
        this.posY = shootingEntity.posY + shootingEntity.getEyeHeight() + direction.y * xoffset + (1 - Math.abs(direction.yCoord)) * yoffset;
        this.posZ = shootingEntity.posZ + direction.zCoord * xoffset - direction.y * horzz * yoffset + horzx * zoffset;
