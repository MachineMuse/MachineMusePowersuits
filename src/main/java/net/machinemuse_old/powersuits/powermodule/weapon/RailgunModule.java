package net.machinemuse_old.powersuits.powermodule.weapon;
...
public class RailgunModule extends PowerModuleBase implements IRightClickModule, IPlayerTickModule {
    ...
    public void drawParticleStreamTo(EntityPlayer source, World world, double x, double y, double z) {
        ...
        double cx = source.posX + direction.x * xoffset - direction.y * horzdir.x * yoffset - horzdir.zCoord * zoffset;
        double cy = source.posY + source.getEyeHeight() + direction.y * xoffset + (1 - Math.abs(direction.yCoord)) * yoffset;
        double cz = source.posZ + direction.zCoord * xoffset - direction.y * horzdir.zCoord * yoffset + horzdir.x * zoffset;
        double dx = x - cx;
        double dy = y - cy;
        double dz = z - cz;
        ...
        if (hand == EnumHand.MAIN_HAND) {
            ...
            if (ElectricItemUtils.getPlayerEnergy(playerIn) > energyConsumption && timer == 0) {
                ...
                if (hitMOP != null) {
                    switch (hitMOP.typeOfHit) {
                        case ENTITY:
                            ...
                            if (hitMOP.entityHit.attackEntityFrom(damageSource, (int) damage)) {
                                hitMOP.entityHit.addVelocity(lookVec.x * knockback, Math.abs(lookVec.y + 0.2f) * knockback, lookVec.zCoord
                                        * knockback);
                            }
                            break;
                        case BLOCK:
                    ...
                    playerIn.addVelocity(-lookVec.x * knockback, Math.abs(-lookVec.y + 0.2f) * knockback, -lookVec.zCoord * knockback);
