...
public abstract class MuseRenderer {
    ...
    public static void drawString(String s, double x, double y, Colour c) {
        // FIXME -- All of this stuff is commented out now in 1.10.2 because it is not needed and causes issues with transparencies of vanilla test, like the playerlevel number
...
//        RenderState.blendingOff();
    ...
    public static void drawLightningTextured(double x1, double y1, double z1, double x2, double y2, double z2, Colour colour) {
        ...
        MuseTextureUtils.pushTexture(MPSResourceConstants.LIGHTNING_TEXTURE);
        RenderState.blendingOn();
        colour.doGL();
        ...
        while (Math.abs(cx) < Math.abs(tx) && Math.abs(cy) < Math.abs(ty) && Math.abs(cz) < Math.abs(tz)) {
            ...
            int index = (int) Math.round(Math.random() * 50); // FIXME: Math.random() cast to int is always rounded down to 0
