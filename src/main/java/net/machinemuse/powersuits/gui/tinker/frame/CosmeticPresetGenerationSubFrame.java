...
// TODO: make this extend PartSpecManipSubFrameBase or make a base class for both to extend

@SideOnly(Side.CLIENT)
public class CosmeticPresetGenerationSubFrame extends PartSpecManipSubFrameBase {
    ...
    public CosmeticPresetGenerationSubFrame(SpecBase model, ColourPickerFrame colourframe, ItemSelectionFrame itemSelector, MuseRelativeRect border) {
    ...
    private List<PartSpecBase> getPartSpecs() {
        return model.apply().values().stream().filter(spec-> isValidItem(getSelectedItem(), spec.getBinding().getSlot())).collect(Collectors.toList());
    }
