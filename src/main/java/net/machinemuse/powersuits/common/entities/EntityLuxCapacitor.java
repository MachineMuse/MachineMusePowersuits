package net.machinemuse.powersuits.common.entities;
...
import static net.machinemuse.powersuits.common.blocks.BlockLuxCapacitor.COLOUR;
import static net.minecraft.block.BlockDirectional.FACING;

public class EntityLuxCapacitor extends EntityThrowable implements IEntityAdditionalSpawnData {
    public EnumColour color;

    public EntityLuxCapacitor(World par1World) {
        super(par1World);
    }

    public EntityLuxCapacitor(World world, EntityLivingBase shootingEntity, EnumColour color) {
                ...
                if (block == null || block.isAir(world.getBlockState(blockPos), world, blockPos)) {
                    ...
                    if (blockToStickTo.isNormalCube(world.getBlockState(blockPos), world, blockPos) &&
                        ...
                        world.setBlockState(blockPos, ((IExtendedBlockState) BlockLuxCapacitor.getInstance().getDefaultState().
                                withProperty(FACING, dir)).withProperty(COLOUR, color));
    ...
    public void writeSpawnData(ByteBuf buffer) {
        if (color != null)
            buffer.writeByte(color.getIndex());
    }

    @Override
    public void readSpawnData(ByteBuf additionalData) {
        this.color = EnumColour.getColourEnumFromIndex(additionalData.readByte());
    }
}