...
public class PartSpecManipSubFrame {
    ...
    public List<PartSpec> partSpecs;
    public boolean open;

    public PartSpecManipSubFrame(Spec model, ColourPickerFrame colourframe, ItemSelectionFrame itemSelector, MuseRelativeRect border) {
        this.model = model;
     ...
     */
    private List<PartSpec> getPartSpecs() {
    ...
    public NBTTagCompound getOrDontGetSpecTag(PartSpec partSpec) {
    ...
    public NBTTagCompound getSpecTag(PartSpec partSpec) {
        NBTTagCompound nbt = getOrDontGetSpecTag(partSpec);
        return nbt != null ? nbt : new NBTTagCompound();
    }

    public NBTTagCompound getOrMakeSpecTag(PartSpec partSpec) {
        String name;
    ...
    public void drawSpecPartial(double x, double y, PartSpec partSpec, double ymino, double ymaxo) {
    ...
    public boolean tryMouseClick(double x, double y) {
        ...
        if (x < this.border.left() || x > this.border.right() || y < this.border.top() || y > this.border.bottom())
            ...
            PartSpec spec = partSpecs.get(Math.max(Math.min(lineNumber, partSpecs.size() - 1), 0));
