...
public class ClientTickHandler {
    ...
    public void findInstalledModules(EntityPlayer player) {
        if (player != null) {
            ...
            if (helmet != null && helmet.getItem() instanceof ItemPowerArmorHelmet) {
                if (ModuleManager.getInstance().itemHasActiveModule(helmet, MPSModuleConstants.MODULE_AUTO_FEEDER)) {
                    modules.add(MPSModuleConstants.MODULE_AUTO_FEEDER);
                }
                if (ModuleManager.getInstance().itemHasActiveModule(helmet, MPSModuleConstants.MODULE_CLOCK)) {
                    modules.add(MPSModuleConstants.MODULE_CLOCK);
                }
                if (ModuleManager.getInstance().itemHasActiveModule(helmet, MPSModuleConstants.MODULE_COMPASS)) {
                    modules.add(MPSModuleConstants.MODULE_COMPASS);
                }
            }

            ItemStack chest = player.getItemStackFromSlot(EntityEquipmentSlot.CHEST);
            if (chest != null && chest.getItem() instanceof ItemPowerArmorChestplate) {
                if (ModuleManager.getInstance().itemHasActiveModule(chest, MPSModuleConstants.MODULE_WATER_TANK)) {
                    modules.add(MPSModuleConstants.MODULE_WATER_TANK);
                }
            }

            ItemStack powerfist = player.getHeldItemMainhand();
            if (powerfist != null && powerfist.getItem() instanceof ItemPowerFist) {
                if (ModuleManager.getInstance().itemHasActiveModule(powerfist, MPSModuleConstants.MODULE_PLASMA_CANNON))
                    modules.add(MPSModuleConstants.MODULE_PLASMA_CANNON);
            }
        }
    }
    ...
    public void onRenderTickEvent(TickEvent.RenderTickEvent event) {
        ...
        double yBaseIcon;
        int yBaseString;
        if (MPSConfig.getInstance().useGraphicalMeters()) {
            yBaseIcon = 150.0;
            yBaseString = 155;
        } else {
            yBaseIcon = 26.0;
        ...
        if (event.phase == TickEvent.Phase.END) {
            ...
            if (player != null && Minecraft.getMinecraft().isGuiEnabled() && MuseItemUtils.modularItemsEquipped(player).size() > 0 && Minecraft.getMinecraft().currentScreen == null) {
                ...
                for (int i = 0; i < modules.size(); i++) {
                    if (Objects.equals(modules.get(i), MPSModuleConstants.MODULE_AUTO_FEEDER)) {
                    ...
                    } else if (Objects.equals(modules.get(i), MPSModuleConstants.MODULE_CLOCK)) {
                        long time = player.world.provider.getWorldTime();
                        long hour = ((time % 24000) / 1000);
                        if (MPSConfig.getInstance().use24hClock()) {
                            if (hour < 19) {
                                hour += 6;
                            } else {
                                hour -= 18;
                    ...
                    } else if (Objects.equals(modules.get(i), MPSModuleConstants.MODULE_COMPASS)) {
                        if (i == 0) {
                            MuseRenderer.drawItemAt(-1.0, yBaseIcon, compass);
                        } else {
                            MuseRenderer.drawItemAt(-1.0, yBaseIcon + (yOffsetIcon * i), compass);
                        }
                    } else if (Objects.equals(modules.get(i), MPSModuleConstants.MODULE_WATER_TANK)) {
                        drawWaterMeter = true;
                    }
                    else if (Objects.equals(modules.get(i), MPSModuleConstants.MODULE_PLASMA_CANNON)) {
