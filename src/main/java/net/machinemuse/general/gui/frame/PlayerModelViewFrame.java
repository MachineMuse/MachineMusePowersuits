 ...
 *
 * This is the part of the GUI that shows the player and equipped items.
 */
...
public class PlayerModelViewFrame implements IGuiFrame {
    ...
    public void draw() {
        ...
        if (getSelectedItem().getItem().getItem() instanceof ItemPowerArmor) {
            HighPolyArmor.getInstance().setRenderSpec(MuseItemUtils.getMuseRenderTag(getSelectedItem().getItem(), getEquipmentSlot()));
            HighPolyArmor.getInstance().setVisibleSection(this.getEquipmentSlot());
