...
public class ClickableKeybinding extends ClickableButton {
    ...
    public void toggleModules() {
        ...
        for (ClickableModule module : boundModules) {
            ...
            if ((FMLCommonHandler.instance().getEffectiveSide().equals(Side.CLIENT) && MPSConfig.getInstance().toggleModuleSpam())) {
                player.sendMessage(new TextComponentString("Toggled " + module.getModule().getUnlocalizedName() + valstring));
            }
            ModuleManager.getInstance().toggleModuleForPlayer(player, module.getModule().getUnlocalizedName(), toggleval);
            MusePacketToggleRequest toggleRequest = new MusePacketToggleRequest(player, module.getModule().getUnlocalizedName(), toggleval);
